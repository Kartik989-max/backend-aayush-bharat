...
import { getFilePreview, storage } from "@/lib/appwrite"; // adjust path to your appwrite config
...
export  function MediaManager({ onSelect, onClose,allowMultiple=false }: MediaManagerProps) {
 ...
 const filteredFiles = files.filter((f) => {
  ...
  const fetchFiles = async () => {
    const res = await storage.listFiles(process.env.NEXT_PUBLIC_APPWRITE_STORAGE_BUCKET_ID!, [
          `limit(${itemsPerPage})`,
...
const toggleSelection = (id: string) => {
  setSelected((prev) => {
    if (allowMultiple) {
      ...
      return prev.includes(id) ? [] : [id]; // only one image at a time
    }
  });
};
